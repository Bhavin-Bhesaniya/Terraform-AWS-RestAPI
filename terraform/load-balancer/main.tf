...
output "aws_lb_dns_name" {
  value = aws_lb.aws_restapi_lb.dns_name
}

output "aws_lb_zone_id" {
  value = aws_lb.aws_restapi_lb.zone_id
}


# Load Balancer configuration
resource "aws_lb" "aws_restapi_lb" {
  name               = var.lb_name
...
resource "aws_lb_target_group_attachment" "aws_restapi_lb_target_group_attachment" {
  target_group_arn = var.lb_target_group_arn
  target_id        = var.ec2_instance_id          # Replace with your EC2 instance reference
  port             = var.lb_target_group_attachment_port
}

resource "aws_lb_listener" "aws_restapi_lb_listner" {
  load_balancer_arn = aws_lb.aws_restapi_lb.arn
...
# resource "aws_lb_listener" "aws_restapi_lb_https_listner" {
#   load_balancer_arn = aws_lb.aws_restapi_lb.arn
